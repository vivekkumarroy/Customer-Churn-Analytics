    ...
    <div className="flex flex-col items-center">
      <div className="relative w-64 h-64">
        <svg className="w-full h-full transform -rotate-90">
          <circle cx="128" cy="128" r={radius} stroke="#334155" strokeWidth="16" fill="transparent" />
          <circle
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-4xl font-black text-theme-text1">{percentage.toFixed(1)}%</span>
          <span className="text-sm font-bold mt-2 tracking-wider" style={{ color }}>{riskText}</span>
        </div>
      </div>
    </div>
  );
...
const Prediction = () => {
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-10 max-w-6xl mx-auto">
      ...
      {/* Header */}
      <div>
        <h1 className="text-3xl font-extrabold text-theme-text1 tracking-tight">Churn Prediction Engine</h1>
        <p className="text-theme-text2 mt-1">Estimate churn probability for a customer using the trained ML model.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          {result ? (
            <div className="dashboard-card p-8 h-full flex flex-col items-center justify-center animate-fade-in relative overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-theme-accent1 to-theme-accent2"></div>

              <h2 className="text-2xl font-bold text-theme-text1 mb-8">Prediction Result</h2>

              <CircularGauge percentage={result.churn_probability * 100} />

              <div className="w-full grid grid-cols-2 gap-4 mt-8">
                <div className="bg-theme-bg2 border border-theme-border rounded-lg p-4 text-center">
                  <p className="text-xs font-semibold text-theme-muted uppercase tracking-wider mb-1">Prediction</p>
                  <p className="text-lg font-bold text-theme-text1">
                    {result.churn_prediction === 1 ? 'CHURN' : 'RETAIN'}
                  </p>
                </div>
                <div className="bg-theme-bg2 border border-theme-border rounded-lg p-4 text-center">
                  <p className="text-xs font-semibold text-theme-muted uppercase tracking-wider mb-1">Threshold</p>
                  <p className="text-lg font-bold text-theme-text1">{result.threshold}</p>
                </div>
              </div>

              <div className="mt-6 w-full text-center">
                <p className="text-xs text-theme-muted uppercase tracking-wider">Model Used</p>
