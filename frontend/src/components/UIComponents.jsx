...
export const MetricCard = ({ title, value, subtitle, icon: Icon }) => (
  <div className="dashboard-card p-6 flex flex-col justify-between h-full group relative overflow-hidden">
    ...
    <div className="flex justify-between items-start mb-6 relative z-10">
      <div>
        <p className="text-xs font-bold text-theme-muted uppercase tracking-widest mb-2">{title}</p>
        <h3 className="text-4xl font-black text-theme-text1 leading-none tracking-tight truncate" title={String(value)}>{value}</h3>
      </div>
      <div className="p-3 bg-theme-bg rounded-xl text-theme-primary shadow-inner border border-theme-border/50 group-hover:bg-theme-primary/20 transition-colors">
